...
export function calculateShipment(shipment) {
  ...
  for (const line of shipment.lines || []) {
    ...
    for (const field of ['grossKg', 'netKg', 'tailGrossKg', 'tailNetKg']) {
    ...
    for (let index = 0; index < count; index++) {
      ...
      const grossG = grams(tail ? line.tailGrossKg : line.grossKg);
      const netG = grams(tail ? line.tailNetKg : line.netKg);
      ...
      if (grossG === null || grossG <= 0) pending.push(`${tag} #${index + 1}: ${tail ? 'measured partial-carton' : 'carton'} gross weight required`);
  ...
  for (const pallet of shipment.pallets || []) {
    ...
    if (tare === null || tare <= 0) pending.push(`${no}: pallet tare weight required`);
    if (extra === null) pending.push(`${no}: film / separators / straps weight required; enter 0 only if confirmed`);
    const cartonG = sumKnown(members.map(c => grams(c.grossKg)));
    const grossG = sumKnown([cartonG, tare, extra]);
    const capacity = grams(pallet.maxGrossKg);
    if (capacity !== null && grossG !== null && grossG > capacity) errors.push(`${no}: loaded pallet exceeds entered gross-weight limit`);
