...
function prepareItem(item, itemIndex, effective, config) {
  ...
  const loadedPalletL = pallet.l + config.palletTolerance;
  const loadedPalletW = pallet.w + config.palletTolerance;
  ...
  return {
    ...
    requiredBoxes,
    requiredUnits,
    requiredVolumeCbm: requiredUnits * pallet.l * pallet.w * stacking.stackHeight / 1_000_000_000,
...
export function planMixedContainers(items, container, config = {}) {
  ...
  while ([...remaining.values()].some((value) => value > 0) && guard < 1000) {
    guard += 1;
    const plan = { index: containers.length + 1, blocks: [], positions: [], usedLength: 0, totalBoxes: 0, totalEa: 0, totalPackingUnits: 0, totalPallets: 0, volumeCbm: 0, skuBoundaryInterlocks: 0 };
    let currentX = 0;
    ...
    for (const item of normalized) {
      ...
      const block = {
        ...
        volumeCbm: item.packaging === "pallet"
          ? loadedUnits * item.pallet.l * item.pallet.w * item.loadingUnit.h / 1_000_000_000
  ...
  return {
    ...
    config: {
      ...
      palletTolerance, palletGap, edgeInset, palletMinHeight, palletHeightLimit, allowDoubleStack, allowSkuInterlock,
