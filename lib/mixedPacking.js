...
export function planMixedContainers(items, container, config = {}) {
  ...
  const palletMinHeight = Number.isFinite(config.palletMinHeight) ? Math.max(1, config.palletMinHeight) : 1200;
  const palletHeightLimit = Number.isFinite(config.palletHeightLimit) ? Math.max(1, config.palletHeightLimit) : 1800;
