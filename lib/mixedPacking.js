 ...
 * If any position cannot be reinserted inside the original effective length,
 * the caller retains the already validated first-pass layout.
 */
function compactContainerTowardFront(plan, effectiveLength, effectiveWidth, skuGap) {
  ...
  const ordered = plan.positions.slice().sort((left, right) =>
    Number(Boolean(left.partialCartonEa)) - Number(Boolean(right.partialCartonEa))
      || (blockRank.get(left.skuId) ?? 0) - (blockRank.get(right.skuId) ?? 0)
      || Number(Boolean(left.rotated)) - Number(Boolean(right.rotated))
      || left.x - right.x
      || left.y - right.y
  ...
  for (const source of ordered) {
    const sourceItem = itemById.get(source.skuId);
    const candidates = new Set([0]);
    for (const obstacle of placed) {
      const obstacleItem = itemById.get(obstacle.skuId);
      const gap = source.skuId === obstacle.skuId
        ? Math.max(sourceItem?.unitGap ?? 0, obstacleItem?.unitGap ?? 0)
        : skuGap;
    ...
    for (const x of [...candidates].sort((left, right) => left - right)) {
      ...
      while (y + source.h <= effectiveWidth + 0.001) {
        ...
        const conflicts = placed.filter((obstacle) => {
          const obstacleItem = itemById.get(obstacle.skuId);
          const gap = candidate.skuId === obstacle.skuId
            ? Math.max(sourceItem?.unitGap ?? 0, obstacleItem?.unitGap ?? 0)
            : skuGap;
          return !positionsSeparated(candidate, obstacle, gap);
        });
        if (!conflicts.length) {
          next = candidate;
          break;
  ...
  const positions = placed.sort((left, right) => left.x - right.x || left.y - right.y);
...
export function planMixedContainers(items, container, config = {}) {
  ...
  while (
    ...
    if (madeProgress) {
      if (allowSkuInterlock && plan.positions.length > 1) {
        const compacted = compactContainerTowardFront(plan, effective.l, effective.w, skuGap);
        if (compacted && compacted.usedLength <= plan.usedLength + 0.05) {
...
export function validateMixedPlan(result) {
  ...
  for (const plan of result.containers) {
    ...
    if (plan.requiresSecuring !== (actualVoids.maximum > 150 + 0.001)) errors.push(`Container ${plan.index}: securing threshold is inconsistent.`);
    ...
    for (const block of plan.blocks.filter((entry) => entry.partialCartonEa)) {
      ...
      const furthestCompatibleEnd = block.positions
        .filter((position) => close(position.w, partial.w, 0.05) && close(position.h, partial.h, 0.05))
        .reduce((maximum, position) => Math.max(maximum, position.x + position.w), 0);
      if (!close(partial.x + partial.w, furthestCompatibleEnd, 0.05)) {
        errors.push(`Container ${plan.index}: ${block.item.code || block.item.id} partial carton is not at the furthest compatible door-side position.`);
      }
    }
  }
...
/**
 * Find the largest whole-product kit quantity that can be loaded into a fixed
...
export function maximizeKitQuantity(items, container, config = {}) {
  ...
  let low = 0;
  let high = theoreticalUpper;
  while (low + 1 < high) {
    const midpoint = Math.floor((low + high) / 2);
    if (evaluate(midpoint).feasible) low = midpoint;
    else high = midpoint;
  }
  ...
  return {
    ...
    residualCapacityVerified: low > 0 && !nextQuantityFeasible,
...
export function optimizeProcurementQuantities(items, container, config = {}) {
  ...
  for (const [key, members] of grouped.entries()) {
    ...
    const maximum = explicitMaximums.length
      ? Math.max(minimum, Math.min(...explicitMaximums))
      : automaticMaximum;
  ...
  const evaluate = (values) => {
    ...
    const utilization = rawCapacity > 0
      ? result.totalRequiredVolumeCbm * 1_000_000_000 / rawCapacity * 100
      : 0;
  ...
  const minimumVector = Object.fromEntries(decisionGroups.map((group) => [group.key, group.minimum]));
  const fixedOnly = evaluate(minimumVector);
  if (!fixedOnly.feasible)
    return {
      ...
      error: fixedOnly.result.unplanned[0]?.reason || fixedOnly.audit.errors[0] || "Minimum/fixed quantities exceed the selected container capacity.",
    };
  ...
  const maximizeGroup = (seed, group) => {
    let low = Math.max(group.minimum, seed[group.key] || 0);
    let high = group.maximum + 1;
    while (low + 1 < high) {
      const midpoint = Math.floor((low + high) / 2);
      const probe = { ...seed, [group.key]: midpoint };
      if (evaluate(probe).feasible) low = midpoint;
      else high = midpoint;
    }
  ...
  return {
    ...
    evaluations: evaluationCache.size,
    residualCapacityVerified: saturationChecks.every((check) => check.reason !== "additional-ea-fits"),
    saturationChecks,
