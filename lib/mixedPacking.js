...
function chooseDoorLayout(plan, effective, skuGap, allowSkuInterlock, separatorThickness) {
  ...
  const candidates = [];
  const cartonRemainderSkus = new Set(plan.positions.filter(p => {
    const item = plan.blocks.find(b => b.item.id === p.skuId)?.item;
    return item?.packaging === "carton" && needsDoorStaging(p,item,effective.h);
  }).map(p => p.skuId));
  // Compare mixed remainders even when a separate-column solution fits.
  // Equal geometric quality still favours simpler, separately labelled loads.
  for (const consolidate of cartonRemainderSkus.size > 1 ? [false, true] : [false]) {
    candidates.push(...variants.flatMap((variant) => {
      ...
      return candidate && auditDoorStaging(candidate,effective.h).ok
        ? [{ candidate, consolidated: consolidate, voids: analyzeFloorVoids(candidate.positions, effective.l) }] : [];
    }));
    if (!consolidate && auditDoorStaging(plan, effective.h).ok && plan.positions.every((p) => p.x + p.w <= effective.l + 0.001)) {
      candidates.push({ candidate: { positions: plan.positions, blocks: plan.blocks, usedLength: plan.usedLength,
        skuBoundaryInterlocks: plan.skuBoundaryInterlocks }, consolidated: false, voids: analyzeFloorVoids(plan.positions, effective.l) });
    }
  }
  candidates.sort((a, b) => a.voids.internalArea - b.voids.internalArea
    ...
    || Number(a.consolidated) - Number(b.consolidated));
