...
function itemRequirementAtQuantity(item, quantity) {
  const cartons = Math.ceil(quantity / item.eaPerBox);
  ...
  if (item.packaging !== "pallet") {
    ...
    return {
      cartons,
      partialCartonEa: quantity % item.eaPerBox,
      pallets: 0,
...
function sampledQuantities(group, target, limit = MAX_OPTIONS_PER_GROUP) {
  const minimum = group.minimumK;
  const maximum = group.maximumK;
  const count = maximum - minimum + 1;
  if (count <= limit) return Array.from({ length: count }, (_, index) => minimum + index);
  const mandatory = new Set([minimum, maximum]);
  const values = new Set([minimum, maximum]);
  const edgeCount = Math.min(40, Math.floor(limit / 10));
  for (let offset = 0; offset < edgeCount; offset += 1) {
    values.add(minimum + offset);
    values.add(maximum - offset);
    mandatory.add(minimum + offset);
    mandatory.add(maximum - offset);
  }
  ...
  // The physical requirement only changes when Q crosses an EA/BOX boundary.
  // Sampling these endpoints covers the meaningful carton vectors without
  // iterating every PCS value; evenly spaced quantities retain useful mix
  // diversity when the legal interval is very large.
  for (const item of group.items) {
    const eaPerBox = item.eaPerBox;
    const first = Math.ceil(minimum / eaPerBox);
    const last = Math.floor(maximum / eaPerBox);
    const boundaryCount = Math.max(0, last - first + 1);
    const samples = Math.min(boundaryCount, Math.max(80, Math.floor(limit / group.items.length)));
    for (let index = 0; index < samples; index += 1) {
      const cartonIndex = samples === boundaryCount
        ? first + index
        : Math.round(first + (last - first) * index / Math.max(1, samples - 1));
      const quantity = cartonIndex * eaPerBox;
      if (quantity >= minimum && quantity <= maximum) values.add(quantity);
    }
  }
  const uniformSamples = Math.min(240, Math.max(2, Math.floor(limit / 4)));
  for (let index = 0; index < uniformSamples; index += 1) {
    values.add(Math.round(minimum + (maximum - minimum) * index / Math.max(1, uniformSamples - 1)));
  }
  const sorted = [...values].filter((value) => value >= minimum && value <= maximum).sort((left, right) => left - right);
  if (sorted.length <= limit) return sorted;
  const selected = new Set([...mandatory].filter((value) => value >= minimum && value <= maximum));
  const optional = sorted.filter((value) => !selected.has(value));
  const remaining = Math.max(0, limit - selected.size);
  for (let index = 0; index < remaining; index += 1) {
    const position = remaining === 1
      ? optional.length - 1
      : Math.round((optional.length - 1) * index / (remaining - 1));
    if (position >= 0) selected.add(optional[position]);
  }
  ...
  for (const candidate of candidates) {
    const nearDuplicate = selected.some((existing) => {
      const totalStepDifference = groups.reduce((sum, group) => sum + Math.abs(
        candidate.groupQuantities[group.id] / group.diversityStep
        - existing.groupQuantities[group.id] / group.diversityStep,
      ), 0);
...
export function validateKitAssignments(items) {
  ...
  return { ok: errors.length === 0, errors, warnings: [], groupCount: grouped.size };
}

/**
 * Optimize equal-PCS kit quantities above the existing physical mixed-loading
 * planner. Product quantities may be any positive integer; carton demand is
 * rounded up independently for every SKU. Every shortlisted result is then
 * re-planned and geometrically audited.
 */
export function optimizeKitPurchases(items, container, config = {}, options = {}) {
  ...
  for (const [id, groupItems] of rawGroups) {
    ...
    const minimum = snapMinimum(constraint.min, 1);
    const requestedMaximum = snapMaximum(constraint.max, 1);
    const group = {
      id,
      items: groupItems,
      step: 1,
      diversityStep: Math.max(1, Math.min(...groupItems.map((item) => item.eaPerBox))),
      constraint,
      minimum,
      minimumK: minimum,
      requestedMaximum,
  ...
  for (const group of groups) {
    ...
    const targetQuantity = Number.isFinite(group.constraint.target) && group.constraint.target > 0
      ? group.constraint.target
      : NaN;
    const quantities = sampledQuantities(group, targetQuantity);
    group.options = quantities
      .map((quantity) => groupRequirementAtQuantity(group, quantity))
      .filter(Boolean);
  ...
  for (const group of groups) {
    combinationCount *= group.options.length;
