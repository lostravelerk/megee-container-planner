...
export default function PalletPolicySummary({ result, language }: { result: PlanResult; language: Language }) {
  ...
  const number = (n: number | null, digits = 0) => n === null ? "—" : n.toLocaleString("en-US", { maximumFractionDigits: digits });
  return <section className="pallet-policy-summary">
    <h3>{tr("包装效率与组托参数", "Packaging efficiency & pallet policy")}</h3>
    <div className="pallet-policy-table-wrap"><table>
      <thead><tr><th>SKU</th><th>EA/BOX</th><th>{tr("外箱 CBM", "Carton CBM")}</th><th>{tr("理论 CBM / 万只", "Theoretical CBM / 10,000 EA")}</th><th>{tr("纸箱层 / 叠托层数", "Carton layers / pallet tiers")}</th><th>{tr("满载单托名义高 / 设计高 mm", "Full pallet nominal / design height mm")}</th></tr></thead>
      <tbody>{result.items.map(item => {
        ...
        return <tr key={item.id}><td>{item.code || item.name || "—"}</td><td>{number(item.eaPerBox)}</td>
          <td>{number(kpi === null ? null : kpi * item.eaPerBox / 10000, 6)}</td><td>{number(kpi, 6)}</td>
          <td>{pallet ? `${item.palletPlan.layersPerPallet} / ${item.palletPlan.stackLevels}` : "—"}</td>
          <td>{pallet ? `${number(item.palletPlan.layersPerPallet * item.carton.h + item.pallet.h, 2)} / ${number(item.palletPlan.stackHeight, 2)}` : "—"}</td></tr>;
      })}</tbody>
    </table></div>
