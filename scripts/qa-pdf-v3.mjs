...
const pdfPath = "/Users/coady/Documents/Codex/CTN QTY/output/reports/404-24牙喷头-20GP最大齐套装柜报告-v3.4.0.pdf";
const packingPdfPath = "/Users/coady/Documents/Codex/CTN QTY/output/reports/404-24牙喷头-20GP装箱单-v3.4.0.pdf";
...
async function main() {
  ...
  await waitFor(
    `document.querySelectorAll('.report-scene-snapshot img[src^="data:image/"]').length === 4`,
    "four realistic report camera plates",
  );
  const preflight = await evaluate(`(async () => {
    ...
    return {
      ...
      realisticViews: report.querySelectorAll('.report-scene-snapshot img[src^="data:image/"]').length,
      perspectiveViews: report.querySelectorAll('.report-scene-perspective img').length,
      topViews: report.querySelectorAll('.report-scene-top img').length,
      sideViews: report.querySelectorAll('.report-scene-side img').length,
      endViews: report.querySelectorAll('.report-scene-door img').length,
  ...
  const packingPdf = await client.send("Page.printToPDF", {
    printBackground: true,
    preferCSSPageSize: true,
    displayHeaderFooter: false,
    transferMode: "ReturnAsBase64",
  });
  await mkdir(join(packingPdfPath, ".."), { recursive: true });
  await writeFile(packingPdfPath, Buffer.from(packingPdf.data, "base64"));
  ...
  client.close();
  process.stdout.write(`${JSON.stringify({ pdfPath, packingPdfPath, preflight, packingPreflight: { ...packingPreflight, text: undefined } })}\n`);
}
